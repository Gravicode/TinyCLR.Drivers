...
namespace Meadow.TinyCLR.ICs.EEPROM
{
    /// <summary>
    ...
    public class At24Cxx
    {
        ...
        public At24Cxx(string i2cBus, byte address = 0x50, ushort pageSize = 32, ushort memorySize = 8192)
        {
        ...
        public byte[] Read(ushort startAddress, ushort amount)
        {
            ...
            _eeprom.Write(address);
            var bytes = new byte[amount];
            _eeprom.Read(bytes,0,amount);
            return bytes;
        }
