    ...
    public class Vc0706Core : IDisposable
    {
      ...
      //  private ManualResetEvent _dataReceivedEvent = new ManualResetEvent(false);
       ...
       //     _comPort.ErrorReceived += new SerialErrorReceivedEventHandler(ErrorReceivedHandler);
        ...
        private void _serialPort_DataReceived(UartController sender, DataReceivedEventArgs e)
        {
            //throw new NotImplementedException();
        }

        public void SetComPort(UartController comPort)
        {
            _serialPort = comPort;
        }
        ...
        public void TakePicture(string path)
        {
            ...
            Debug.WriteLine("Frame length: " + frameLength.ToString());
            ...
            using (var picFile = new FileStream(path, FileMode.Create, FileAccess.Write))
            {
                ...
                Debug.WriteLine("Total bytes read: " + totalBytesRead.ToString());
        ...
        {
            var timeout = ((bufferLength / (this.BaudRate >> 3)) * 1000) + CameraDelayMilliSec + TimeoutToleranceMilliSec;
            return timeout;
        }
        ...
        protected void RunCommand(Command command, byte[] args, int expectedResponseLength, UartController comPort, bool flushComPort = true)
        {
            if (flushComPort)
            { 
