...
namespace Meadow.TinyCLR.Displays
{
    ...
    public sealed class Tm1637
    {
        ...
        bool isExists(byte[] data, int num)
        {
        ...
        private readonly GpioPin portClock;
        private readonly GpioPin portData;
        ...
        public Tm1637(int pinClock, int pinData)
        {
            var gpio = GpioController.GetDefault();
            portClock = gpio.OpenPin(pinClock);
            portClock.SetDriveMode(GpioPinDriveMode.Output);

            portData = gpio.OpenPin(pinData);
            portData.SetDriveMode(GpioPinDriveMode.Output);
        ...
        private bool WriteByte(byte data)
        {
            for (byte i = 0; i < 8; i++)
            {
                portClock.Write(GpioPinValue.Low);// false;
                Thread.Sleep(1); //wait at least 1 micro second

                // LSB first
                if ((data & 0x01) == 0x01)
                {
                    portData.Write(GpioPinValue.High);// true;
                }
                else
                {
                    portData.Write(GpioPinValue.Low);// false;
                }
                    
                // LSB first
                data >>= 1;
                portClock.Write(GpioPinValue.High);// = true;
            ...
            portData.SetDriveMode(GpioPinDriveMode.Input);// = PortDirectionType.Input;
            Thread.Sleep(1); // at least 1 micro second

            var response = portData.Read() == GpioPinValue.High;

            if (response == false)
            {
                portData.SetDriveMode(GpioPinDriveMode.Output);// = PortDirectionType.Output;
                portData.Write(GpioPinValue.Low);// = false;
            }
        ...
        {
            portClock.Write(GpioPinValue.Low);// = false;
            portData.Write(GpioPinValue.Low);//.State = false;
            Thread.Sleep(1);
            portData.Write(GpioPinValue.Low);// = false;
            portClock.Write(GpioPinValue.High);// = true;
            portData.Write(GpioPinValue.High);//.State = true;
        }
        ...
        public void Show(Character[] data)
        {
            Show(data);
        }
