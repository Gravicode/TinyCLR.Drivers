...
namespace Meadow.Foundation.Sensors.Atmospheric
{
        ...
        IAtmosphericSensor
    {
        ...
        /// </summary>
        private readonly I2cDevice mpl3115a2;
        ...
        public bool Standby
        {
            ...
            {
                var data = new byte[1];
                mpl3115a2.WriteRead(new byte[] { Registers.Control1 }, data);
                return (data[0] & 0x01) > 0;
            }
            set
            {
                var status = new byte[1];
                mpl3115a2.WriteRead(new byte[] { Registers.Control1 }, status);
                if (value)
                {
                    status[0] &= (byte)~ControlRegisterBits.Active;
                }
                else
                {
                    status[0] |= ControlRegisterBits.Active;
                }
                mpl3115a2.Write(new byte[] { Registers.Control1, status[0] });
            }
        }
        ...
        public byte Status
        {
            ...
            {
                var data = new byte[1];
                mpl3115a2.WriteRead(new byte[] { Registers.Status }, data);
                return data[0];
            }
        }
        ...
        public Mpl3115a2(string i2cBus, byte address = 0x60)
        {
            ...
            mpl3115a2 = controller.GetDevice(settings);
            //mpl3115a2 = new I2cPeripheral(i2cBus, address);
            var reg = new byte[1];
            mpl3115a2.WriteRead(new byte[] { Registers.WhoAmI }, reg);
            if (reg[0] != 0xc4)
            {
                throw new Exception("Unexpected device ID, expected 0xc4");
            }
            mpl3115a2.Write(new byte[]{Registers.Control1,
                                     (byte)(ControlRegisterBits.Active |
                                            ControlRegisterBits.OverSample128) });

            mpl3115a2.Write(new byte[]{Registers.DataConfiguration,
        ...
        protected AtmosphericConditions ReadSensor()
        {
            ...
            Thread.Sleep(100);
            var data = new byte[5];
            mpl3115a2.WriteRead(new byte[] { Registers.PressureMSB }, data);
        ...
        public void Reset()
        {
            ...
            mpl3115a2.WriteRead(new byte[] { Registers.Control1 }, data);
            data[0] |= 0x04;
            mpl3115a2.Write(new byte[] { Registers.Control1, data[0] });
        }
