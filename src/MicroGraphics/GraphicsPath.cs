...
namespace Meadow.Foundation.Graphics
{
    ...
    {
        public Point LastPoint => ((PathAction)PathActions[PathActions.Count-1]).PathPoint;
        ...
        internal ArrayList PathActions { get; private set; } = new  ArrayList();
        ...
        public void RelativeMoveTo(int x, int y)
        {
            ...
            if(count > 0)
            {
                PathActions.Add(new PathAction(new Point(x, y) + ((PathAction)PathActions[count - 1]).PathPoint, VerbType.Move));
            }
            else
            {
                MoveTo(x, y);
        ...
        public void RelativeMoveTo(Point point)
        {
            ...
            if (count > 0)
            {
                PathActions.Add(new PathAction(point + ((PathAction)PathActions[count - 1]).PathPoint, VerbType.Move));
            }
            else
            {
                MoveTo(point);
        ...
        {
            return (PathAction)PathActions[PathActions.Count-1];
        }
