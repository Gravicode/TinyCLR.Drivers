...
namespace Meadow.TinyCLR.Sensors.Camera
{
    public class ArducamMini
    {
        ...
        protected byte ReadSpiRegister(byte address)
        {
            ...
            spiDevice.Write(new byte[] { address });
            var readByte = new byte[1];
            spiDevice.Read(readByte);                
            return readByte[0];//spiDevice.ReadBytes(1)[0];
        }

        protected void WriteSpiRegister(byte address, byte value)
        {
            spiDevice.Write(new byte[] { address, value });
        } 
        ...
        {
            var reg = new byte[1];
            i2cDevice.WriteRead(new byte[] { address },reg);
            return reg[0];
        }
        ...
        private void Initialize()
        {
            ...
            Debug.WriteLine("OV2640_JPEG_INIT...");
            WriteI2cRegisters(InitSettings.JPEG_INIT);

            Debug.WriteLine("OV2640_YUV422...");
            WriteI2cRegisters(InitSettings.YUV422);

            Debug.WriteLine("OV2640_JPEG...");
            ...
            Debug.WriteLine("OV2640_320x240_JPEG");
        ...
        {
            var readByte = new byte[1];
            spiDevice.TransferFullDuplex(new byte[] { SINGLE_FIFO_READ },readByte);
            return readByte[0];
        }

        public byte[] GetImageData()
        {
            Debug.WriteLine("GetImageData");

            Debug.WriteLine($"Len: {ReadFifoLength()}");
