...
namespace Meadow.TinyCLR.Modules.Led
{
    /// <summary>
    ...
    public class PwmLedBarGraph
    {
        ...
        /// </summary>
        public PwmLedBarGraph(string PwmControllerName, int[] pins, float forwardVoltage)
        {
            pwmLeds = new PwmLed[pins.Length];

            for (int i = 0; i < pins.Length; i++)
            {
                pwmLeds[i] = new PwmLed(PwmControllerName, pins[i], forwardVoltage);
            }
        }
        ...
        /// </summary>
        public PwmLedBarGraph(PwmChannel[] ports, float forwardVoltage)
        {
