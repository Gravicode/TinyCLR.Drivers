...
namespace Meadow.TinyCLR.ICs.IOExpanders
{
    ...
    public class Is31fl3731
    {
        ...
        protected readonly I2cDevice i2cPeripheral;
        ...
        {
            SelectPage(frame);
            byte[] ret = new byte[] { reg,data };
            i2cPeripheral.Write(ret);
        }
        ...
        {
            frame = (byte) Math.Max(frame, 0);
            frame = (byte) Math.Min(frame, 7);
        ...
        {
            var bytes = new byte[] { CommandRegister, page };
            i2cPeripheral.Write(bytes);
        }
    }
