...
namespace TinyCLR.Drivers.Generators
{
    /// <summary>
    ...
    {
        protected GpioPin Port { get; set; }
        ...
        //public SoftPwm(IIODevice device, int outputPin, float dutyCycle = 0.5f, float frequency = 1.0f) :
        ...
        /// <param name="dutyCycle"></param>
        /// <param name="frequency"></param>
        public SoftPwmPort(GpioPin outputPort, float dutyCycle = 0.0f, float frequency = 100f)
        {
            Port = outputPort;
            DutyCycle = dutyCycle;
            Frequency = frequency;
        ...
        public void Start()
        {
            ...
            th = new Thread(() =>
            {
                while (running)
                {
                    Port.Write(GpioPinValue.High);
                    Thread.Sleep(onTimeMilliseconds);
                    Port.Write(GpioPinValue.Low);// = false;
