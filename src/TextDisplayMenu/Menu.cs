...
namespace Meadow.Foundation.Displays.TextDisplayMenu
{
    public class Menu
    {
        ...
        {
            var menuString = System.Text.Encoding.UTF8.GetString(menuJson);
        ...
        {
            if (!IsEnabled) {
                Debug.WriteLine("Render not enabled");
                return;
            }
            ...
            for (int i = topDisplayLine; i <= (topDisplayLine + display.DisplayConfig.Height - 1); i++)
            {
                ...
                {
                    item = (MenuItem)currentMenuPage.MenuItems[i];
        ...
        protected string GetItemText(MenuItem item, bool isSelected)
        {
            if(item == null)
            {
                Debug.WriteLine("GetItemText: item is null");
                return "no item";
            }
        ...
        public bool Select()
        {
            ...
            MenuItem menuItem = (MenuItem)currentMenuPage.MenuItems[pos];
