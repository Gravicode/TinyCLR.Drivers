...
namespace Meadow.TinyCLR.Displays
{
    ...
    public class SerialLcd : ITextDisplay
    {
        ...
        /// </summary>
        private readonly UartController comPort;
        ...
        public void WriteLine(string text, byte lineNumber)
        {
            ...
            {
                Debug.WriteLine("Text length exceeds number of columns, truncating.");
                Debug.WriteLine("Text length: " + text.Length.ToString() + ", Columns: " + DisplayConfig.Width.ToString());
                //throw new Exception("Number characters must be <= columns");
                lineText = text.Substring(0, DisplayConfig.Width);
                Debug.WriteLine("Truncating text to: " + lineText);
            }
