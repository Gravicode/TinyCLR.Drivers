...
namespace Meadow.TinyCLR.Sensors.Light
{
    public class Si1145
    {
        ...
        public Si1145(string i2cBus, byte address = 0x60)
        {
            ...
            si1145 = controller.GetDevice(settings);
            //si1145 = new I2cPeripheral(i2cBus, address);
            var read = new byte[1];
            si1145.WriteRead(new byte[] { REG_PARTID }, read);
            if (read[0] != 0x45)
            {
        ...
        public double GetUltraViolet()
        {
            byte[] data = new byte[2];
                si1145.WriteRead(new byte[] { 0x2C }, data);
        ...
        {
            byte[] data = new byte[2];
            si1145.WriteRead(new byte[] { 0x22 }, data);
            int result = (data[1] << 8) | data[0];
            return result;
        }
        ...
        {
            byte[] data = new byte[2];
            si1145.WriteRead(new byte[] { 0x24 }, data);
        ...
        {
            byte[] data = new byte[2];
            si1145.WriteRead(new byte[] { 0x26 }, data);
        ...
        private byte WriteParam(byte param, int value)
        {
            ...
            var read = new byte[1];
            si1145.WriteRead(new byte[] { REG_PARAMRD },read);
            return read[0];
        }

        private void Write8(byte reg, byte val)
        {
            si1145.Write (new byte[] { reg });
            si1145.Write (new byte[]{val});
        }
