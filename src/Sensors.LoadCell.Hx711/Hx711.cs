...
using Meadow.TinyCLR.Helpers.Measurements;
using System;
using System.Diagnostics;
using System.Threading;

namespace Meadow.TinyCLR.Sensors.LoadCell
{
    /// <summary>
    ...
    public class Hx711 : IDisposable
    {
        ...
        public Hx711( int sck, int dout)
        {
            var gpio = GpioController.GetDefault();
            this.sck = gpio.OpenPin(sck);
            this.sck.SetDriveMode(GpioPinDriveMode.Output);
            this.sck.Write(GpioPinValue.Low);
            //this.sck = device.CreateDigitalOutputPort(sck);

            this.dout = gpio.OpenPin(dout);
            this.dout.SetDriveMode(GpioPinDriveMode.Input);
        ...
        public Hx711(GpioPin sck, GpioPin dout)
        {
            this.sck = sck;
            this.dout = dout;

            CalculateRegisterValues(sck.PinNumber, dout.PinNumber);
            Start();
        }
        ...
        {
            tareValue = ReadADC();
            Debug.WriteLine($"Tare base = {tareValue}");
        }
        ...
        public Weight GetWeight()
        {
            ...
            return new Weight((float)grams, WeightUnits.Grams);
        }

        private void CalculateRegisterValues(int sck, int dout, int sck_portoffset=0, int dout_portoffset=0)
        {
            ...
            int gpio_port = sck_portoffset;//sck.Key.ToString()[1] - 'A';
            sck_pin = sck;//int.Parse(sck.Key.ToString().Substring(2));
            sck_address = GPIO_BASE | (0x400u * (uint)gpio_port) | BSSR_OFFSET;
            sck_set = 1u << sck_pin;
            sck_clear = 1u << (16 + sck_pin);

            gpio_port = dout_portoffset;//dout.Key.ToString()[1] - 'A';
            var gpio_pin = dout;// int.Parse(dout.Key.ToString().Substring(2));
