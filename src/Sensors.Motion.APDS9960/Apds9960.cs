...
namespace Meadow.TinyCLR.Sensors.Motion
{
    public class Apds9960
    {
        ...
        /// </summary>
        private readonly I2cDevice apds9960;

        GpioPin interruptPort;
        ...
        public Apds9960(string i2cBus, int interruptPin, byte address = 0x39)
        {
            ...
            apds9960 = controller.GetDevice(settings);
            ...
            if (interruptPin != -1)
            {
                ...
                interruptPort = gpio.OpenPin(interruptPin);
                interruptPort.SetDriveMode(GpioPinDriveMode.InputPullUp);
                interruptPort.ValueChanged += InterruptPort_ValueChanged; 
        ...
        private void InterruptPort_ValueChanged(GpioPin sender, GpioPinValueChangedEventArgs e)
        {
        ...
        {
            var id = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_ID }, id);

         //  if ((id != APDS9960_ID) && (id != APDS9960_ID_1) && (id != APDS9960_ID_2))
       //     if(id == 0)
        //    {
                Debug.WriteLine($"Device found {id[0]}");
       //     }

            Debug.WriteLine("SetMode");
            SetMode(ALL, OFF);

            /* Set default values for ambient light and proximity registers */
            apds9960.Write(new byte[] { APDS9960_ATIME, DEFAULT_ATIME });
            apds9960.Write(new byte[]{APDS9960_WTIME, DEFAULT_WTIME });
            apds9960.Write(new byte[]{APDS9960_PPULSE, DEFAULT_PROX_PPULSE });
            apds9960.Write(new byte[]{APDS9960_POFFSET_UR, DEFAULT_POFFSET_UR });
            apds9960.Write(new byte[]{APDS9960_POFFSET_DL, DEFAULT_POFFSET_DL });
            apds9960.Write(new byte[]{APDS9960_CONFIG1, DEFAULT_CONFIG1 });
            SetLEDDrive(DEFAULT_LDRIVE);

            Debug.WriteLine("SetProximityGain");
            ...
            apds9960.Write(new byte[] { APDS9960_PERS, DEFAULT_PERS });
            apds9960.Write(new byte[]{APDS9960_CONFIG2, DEFAULT_CONFIG2});
            apds9960.Write(new byte[] { APDS9960_CONFIG3, DEFAULT_CONFIG3 });

            Debug.WriteLine("SetGestureEnterThresh");
            SetGestureEnterThresh(DEFAULT_GPENTH);
            
            SetGestureExitThresh(DEFAULT_GEXTH);
            
            apds9960.Write(new byte[] { APDS9960_GCONF1, DEFAULT_GCONF1 });
            ...
            apds9960.Write(new byte[]{APDS9960_GOFFSET_U, DEFAULT_GOFFSET});
            apds9960.Write(new byte[]{APDS9960_GOFFSET_D, DEFAULT_GOFFSET});
            apds9960.Write(new byte[]{APDS9960_GOFFSET_L, DEFAULT_GOFFSET});
            apds9960.Write(new byte[] { APDS9960_GOFFSET_R, DEFAULT_GOFFSET });
            apds9960.Write(new byte[] { APDS9960_GPULSE, DEFAULT_GPULSE });
            apds9960.Write(new byte[] { APDS9960_GCONF3, DEFAULT_GCONF3 });
        ...
        {
            var read = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_ENABLE },read);
            return read[0];
        }
        ...
        void SetMode(byte mode, byte enable)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_ENABLE, reg_val });
        }
        ...
        public bool EnableGestureSensor(bool interrupts)
        {
            ...
            */
            Debug.WriteLine("ResetGestureParameters");
            ResetGestureParameters();
            apds9960.Write(new byte[] { APDS9960_WTIME, 0xFF });
            apds9960.Write(new byte[] { APDS9960_PPULSE, DEFAULT_GESTURE_PPULSE });

            Debug.WriteLine("SetLEDBoost");
            SetLEDBoost(LED_BOOST_100);

            Debug.WriteLine("SetGestureIntEnable");
            SetGestureIntEnable((byte)(interrupts?1:0));

            Debug.WriteLine("SetGestureMode");
            SetGestureMode(1);
            EnablePower(true);
            SetMode(WAIT, 1);
            SetMode(PROXIMITY, 1);
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GSTATUS },val);
        ...
        public Direction ReadGesture()
        {
            ...
            {
                Debug.WriteLine("Read Gesture failed");
                return (int)Direction.NONE;
            }

            Debug.WriteLine("ReadGesture");
            ...
            while (true)
            {
                ...
                Debug.WriteLine("Read: APDS9960_GSTATUS");
                var gstatus = new byte[1];
                 apds9960.WriteRead(new byte[] { APDS9960_GSTATUS },gstatus);

                /* If we have valid data, read in FIFO */
                Debug.WriteLine("Read: APDS9960_GVALID");
                if ((gstatus[0] & APDS9960_GVALID) == APDS9960_GVALID)
                {
                    Debug.WriteLine("Read: APDS9960_GFLVL");

                    fifo_level = new byte[1];
                    apds9960.WriteRead(new byte[] { APDS9960_GFLVL }, fifo_level);

                    /* If there's stuff in the FIFO, read it into our data block */
                    if (fifo_level[0] > 0)
                    {
                        Debug.WriteLine($"fifo level {fifo_level}");

                        byte[] fifo_data = new byte[(ushort)(fifo_level[0] * 4)];
                        apds9960.WriteRead(new byte[] { APDS9960_GFIFO_U }, fifo_data );

                        Debug.WriteLine(BitConverter.ToString(fifo_data));

                        bytes_read = (byte)fifo_data.Length;

                        Debug.WriteLine($"Fifo bytes read {bytes_read}");
                        ...
                        if (bytes_read >= 4)
                        {
                            ...
                            Debug.WriteLine("ProcessGestureData");
                            if (ProcessGestureData())
                            {
                                Debug.WriteLine("DecodeGesture");
                                if (DecodeGesture())
                                {
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_CDATAL },val);

            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_CDATAH },val_byte);

            return (ushort)(val[0] + (val_byte[0] << 8));
        }

        public ushort ReadRedLight()
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_RDATAL },val);

            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_RDATAH }, val_byte);
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GDATAL }, val);

            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GDATAH }, val_byte);
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_BDATAL }, val);

            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_BDATAH }, val_byte);
        ...
        {
            var read = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_PDATA },read);
            return read[0];
        }
        ...
        {
            var read = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_PILT },read);
            return read[0];
        }
        ...
        public void SetProxIntLowThresh(byte threshold)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_PILT, threshold });
        }
        ...
        {
            var read = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_PIHT }, read);
            return read[0];
        ...
        {
            apds9960.Write(new byte[] { APDS9960_PIHT, threshold });
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_CONTROL }, val);
            

            /* Shift and mask out LED drive bits */
            val[0] = (byte)((val[0] >> 6) & 0b00000011);
        ...
        public bool SetLEDDrive(byte drive)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_CONTROL, val[0] });
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_CONFIG2 }, val);
            ...
            apds9960.Write(new byte[] { APDS9960_CONFIG2, val[0] });
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_CONFIG3 }, val);
            //byte val = apds9960.ReadRegister(APDS9960_CONFIG3);

            /* Shift and mask out PCMP bits */
            val[0] = (byte)((val[0] >> 5) & 0b00000001);
        ...
        void SetProxGainCompEnable(byte enable)
        {
            ...
            apds9960.Write(new byte[]{APDS9960_CONFIG3, val[0]});
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GPENTH }, val);
        ...
        {
            apds9960.Write(new byte[] { APDS9960_GPENTH, threshold });
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GEXTH }, val);
        ...
        {
            apds9960.Write(new byte[] { APDS9960_GEXTH, threshold });
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GCONF2 }, val);
            //byte val = apds9960.ReadRegister(APDS9960_GCONF2);

            /* Shift and mask out GGAIN bits */
            val[0] = (byte)((val[0] >> 5) & 0b00000011);
        ...
        void SetGestureGain(byte gain)
        {
            ...
            apds9960.Write(new byte[]{APDS9960_GCONF2, val[0]});
        }
        ...
        {
            var threshold = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_AILTL }, threshold);
            //var threshold = apds9960.ReadRegister(APDS9960_AILTL);

            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_AILTH }, val_byte);
            //var val_byte = apds9960.ReadRegister(APDS9960_AILTH);
      
            return (byte)(threshold[0] + ((ushort)val_byte[0] << 8));
        }
        ...
        void SetLightIntLowThreshold(ushort threshold)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_AILTL, val_low });
            apds9960.Write(new byte[] { APDS9960_AILTL, val_high });
        }
        ...
        {
            var threshold = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_AIHTL }, threshold);
            //var threshold = apds9960.ReadRegister(APDS9960_AIHTL);
            var val_byte = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_AIHTH }, val_byte);
        ...
        void SetLightIntHighThreshold(ushort threshold)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_AIHTL, val_low });
            apds9960.Write(new byte[] { APDS9960_AIHTH, val_high });
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_ENABLE }, val);
            //byte val = apds9960.ReadRegister(APDS9960_ENABLE);

            /* Shift and mask out AIEN bit */
            val[0] = (byte)((val[0] >> 4) & 0b00000001);
        ...
        void SetAmbientLightIntEnable(bool enable)
        {
            ...
            apds9960.Write(new byte[] { APDS9960_ENABLE, val[0]});
        }
        ...
        {
            var val = new byte[1];
            apds9960.WriteRead(new byte[] { APDS9960_GCONF4 }, val);
            //byte val = apds9960.ReadRegister(APDS9960_GCONF4);

            /* Shift and mask out GIEN bit */
            val[0] = (byte)((val[0] >> 1) & 0b00000001);
        ...
        void SetGestureIntEnable(byte enable)
        {
            ...
            apds9960.Write(new byte[]{APDS9960_GCONF4, val[0]});
        }
        ...
        {
            apds9960.Write(new byte[] { APDS9960_AICLEAR, 0 });
        }
        ...
        {
            apds9960.Write(new byte[] { APDS9960_PICLEAR, 0 });
        }
