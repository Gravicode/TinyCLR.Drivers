...
namespace Meadow.TinyCLR.Sensors.Motion
{
    ...
    public class Hmc5883
    {
        ...
        protected static I2cDevice i2cPeripheral;
        ...
        public Hmc5883(string i2cBus, byte address = 0x1E,
        ...
        protected virtual void Initialize()
        {
            ...
            i2cPeripheral.Write(new byte[] { (byte)Register.HMC_CONFIG_REG_A_ADDR, configA});
            i2cPeripheral.Write(new byte[] { (byte)Register.HMC_CONFIG_REG_B_ADDR, configB});
            i2cPeripheral.Write(new byte[] { (byte)Register.HMC_MODE_REG_ADDR, measuringMode});
        }
        ...
        {
            var read = new byte[1];
            i2cPeripheral.WriteRead(new byte[] { (byte)Register.HMC_X_MSB_REG_ADDR }, read);
            ushort x = read[0];

            i2cPeripheral.WriteRead(new byte[] { (byte)Register.HMC_Y_MSB_REG_ADDR }, read);
            ushort y = read[0];

            i2cPeripheral.WriteRead(new byte[] { (byte)Register.HMC_Z_MSB_REG_ADDR }, read);
            ushort z = read[0];
        ...
        private Status GetStatus()
        {
            ...
            //return read[0];
            i2cPeripheral.Write(new byte[] { (byte)Register.HMC_STATUS_REG_ADDR });
            var status = new byte[1];
            i2cPeripheral.Read(status);
