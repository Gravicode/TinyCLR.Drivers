...
namespace Meadow.TinyCLR.Sensors.Camera
{
    /// <summary>
    ...
    public class Mlx90640
    {
        ...
        readonly I2cDevice i2CPeripheral;

        float emissivity;

        public Mlx90640(string i2cBus, byte address = 0x33, Units measurementUnit = Units.Celsius, float emissivity = 0.95f)
        {
        ...
        protected virtual void I2CRead(int startAddress, ushort readLen, ref ushort[] data)
        {
            ...
            while (readLen > 0)
            {
                ...
                var writeBuffer = (cmd);
                var tempBuf = new byte[toRead16 * 2];
        ...
        void ExtractKtaPixelParameters(ref ushort[] eeData)
        {
            ...
            temp = (float) Math.Abs(ktaTemp[0]);
            for (int i = 1; i < 768; i++)
            {
                if (Math.Abs(ktaTemp[i]) > temp)
                    temp = (float)Math.Abs(ktaTemp[i]);
        ...
        void ExtractKvPixelParameters(ref ushort[] eeData)
        {
            ...
            temp =(float) Math.Abs(kvTemp[0]);
            for (int i = 1; i < 768; i++)
            {
                if (Math.Abs(kvTemp[i]) > temp)
                    temp =(float) Math.Abs(kvTemp[i]);
        ...
        void ExtractDeviatingPixels(ref ushort[] eeData)
        {
            ...
            if(Config.BrokenPixels.Count > 1 )
            {
                for (pixCnt = 0; pixCnt < Config.BrokenPixels.Count; pixCnt++)
                {
                    ...
                    {
                        if (!CheckAdjacentPixels((ushort)Config.BrokenPixels[pixCnt], (ushort)Config.BrokenPixels[i]))
                        {
                ...
                for (pixCnt = 0; pixCnt < Config.OutlierPixels.Count; pixCnt++)
                {
                    ...
                    {
                        if (!CheckAdjacentPixels((ushort)Config.OutlierPixels[pixCnt], (ushort)Config.OutlierPixels[i]))
                        {
                ...
                for (pixCnt = 0; pixCnt < Config.BrokenPixels.Count; pixCnt++)
                {
                    ...
                    {
                        if (!CheckAdjacentPixels((ushort)Config.BrokenPixels[pixCnt], (ushort)Config.OutlierPixels[i]))
                        {
