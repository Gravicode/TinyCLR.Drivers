...
namespace Meadow.TinyCLR.DataLoggers
{
            ...
            while (retryCount < 3)
            {
                try
                {
                    ...
                    using (var stream = response.GetResponseStream())
                    {
                        do
                        {
                            read = stream.Read(results, 0, results.Length);
                            total += read;

                            System.Diagnostics.Debug.WriteLine("read : " + read);
                            System.Diagnostics.Debug.WriteLine("total : " + total);

                            resultStr += new String(System.Text.Encoding.UTF8.GetChars
                                (results, 0, read));

                            //System.Diagnostics.Debug.WriteLine("Response : " + resultStr);
                        }

                        while (read != 0);
                    }
                    //var responseString = new StreamReader(response.GetResponseStream()).ReadToEnd();
                    result = int.Parse(resultStr.Trim());
                    retryCount = 4;
                }
                catch
                {
