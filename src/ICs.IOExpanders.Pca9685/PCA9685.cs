...
namespace Meadow.TinyCLR.ICs.IOExpanders
{
    ...
    {
        private readonly I2cDevice i2cPeripheral;
        ...
        public string i2CBus { get; protected set; }
        public byte Address { get; protected set; }

        public Pca9685(string i2cBus, byte address, int frequency = 100)
        {
        ...
        {
            i2cPeripheral.Write( new byte[] { Address, Mode1, 0X00 });
            i2cPeripheral.Write(new byte[] { Address, Mode1 });
        ...
        /// <summary>
        /// Create a PwmChannel on the specified pin
        /// </summary>
        ...
        public virtual IPwmPort CreatePwmPort(byte portNumber, float dutyCycle = 0.5f)
        {
            ...
            var pwmPort = new PwmPortPCA9685(i2cPeripheral, Address, Led0OnL, frequency, portNumber, dutyCycle);
        ...
        {
            i2cPeripheral.Write(new byte[] { Address, register, ledXOnL, ledXOnH, ledXOffL, ledXOffH });
        }

        protected virtual void Write(byte register, byte value)
        {
            i2cPeripheral.Write(new byte[] { register, value });
        }
        ...
        protected virtual void SetFrequency(int frequency)
        {
            ...
            var oldmode = new byte[1];
            i2cPeripheral.WriteRead(new byte[] { Mode1 } , oldmode);
            byte newmode = (byte)((oldmode[0] & 0x7F) | 0x10);         //   # sleep

            Write(Mode1, newmode);//       # go to sleep
            Write(PreScale, (byte)((int)(Math.Floor(prescale))));
            Write(Mode1, oldmode[0]);

            Thread.Sleep(5);
            Write(Mode1, (byte)(oldmode[0] | 0x80 | mode1AI));
        }
    }
}